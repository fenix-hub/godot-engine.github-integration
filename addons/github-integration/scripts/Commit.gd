...
func _on_Button_pressed():
	...
	FILTERS = _filters.text.rsplit(",")
	ONLY = _only.text.rsplit(",")
	
	print(ONLY[0]," ",FILTERS)
...
func request_commit():
	...
	for file in files:
		...
		for content in branches_contents:
			if content.path == file[0].lstrip(DIRECTORY):
				sha = content.sha
		...
		new_repo.request("https://api.github.com/repos/"+UserData.USER.login+"/"+repo_selected.name+"/contents/"+file[0].lstrip(DIRECTORY),UserData.header,false,HTTPClient.METHOD_PUT,JSON.print(bod))
