...
onready var signin_request : HTTPRequest = $SignInRequest
onready var download_image : HTTPRequest = $DownloadRequest
...
func _on_user_requested(user : Dictionary) -> void:
	Error.hide()
	user_data = user
	RestHandler.request_user_avatar(user_data.avatarUrl)
