...
# Collaborators
onready var AddCollaboratorBtn : Button = $Repository/RepoInfos/RepoInfosContainer/repo_infos/AddCollaboratorBtn
...
func set_darkmode(darkmode : bool):
	if darkmode:
		...
		$Repository/RepositoryInfo/RepositoryContent/contents.set("custom_styles/bg", load("res://addons/github-integration/resources/styles/ContentesBG-dark.tres"))   
		$Repository/RepositoryInfo/RepositoryContent/contents.set("custom_styles/bg_focus", load("res://addons/github-integration/resources/styles/ContentesBG-dark.tres"))   
	else:
		$BG.color = "#f6f8fa"
		...
		$Repository/RepositoryInfo/RepositoryContent/contents.set("custom_styles/bg", load("res://addons/github-integration/resources/styles/ContentesBG-white.tres"))
		$Repository/RepositoryInfo/RepositoryContent/contents.set("custom_styles/bg_focus", load("res://addons/github-integration/resources/styles/ContentesBG-white.tres"))
...
func open_repository(repository_item : RepositoryItem) -> void:
	...
	branches.clear()
	contents.clear()
	print(repository_item._repository)
	...
	request_branches(repository.name)
...
func request_branches(rep : String):
	...
	request.request("https://api.github.com/repos/"+owner_lbl.text+"/"+rep+"/branches",UserData.header,false,HTTPClient.METHOD_GET,"")
	yield(self,"get_branches")
	if branches.size() > 0:
			requesting = REQUESTS.TREES
			for b in branches:
					request.request("https://api.github.com/repos/"+owner_lbl.text+"/"+rep+"/branches/"+b.name,UserData.header,false,HTTPClient.METHOD_GET,"")
					yield(self,"get_branches_contents")
			var i = 0
			for branch in branches_contents:
					branches_opt_btn.add_item(branch.name)
					branches_opt_btn.set_item_metadata(i,branch)
					from_branch.add_item(branch.name)
					from_branch.set_item_metadata(i,branch)
					i+=1
			current_branch = branches_opt_btn.get_item_metadata(0)
			request_contents(current_repo._name,branches_opt_btn.get_item_metadata(0))
...
func request_contents(rep : String, branch):
	...
	request.request("https://api.github.com/repos/"+owner_lbl.text+"/"+rep+"/git/trees/"+branch.commit.commit.tree.sha+"?recursive=1",UserData.header,false,HTTPClient.METHOD_GET,"")
...
func delete_repo():
	...
	confirm.popup()
	confirm.connect("confirmed",self,"request_delete",[current_repo._name])

func request_delete(repo : String):
	get_parent().loading(true)
	requesting = REQUESTS.DELETE
	request.request("https://api.github.com/repos/"+owner_lbl.text+"/"+repo,UserData.header,false,HTTPClient.METHOD_DELETE,"")
...
func request_delete_resource(path : String, item : TreeItem = null):
	...
	request.request("https://api.github.com/repos/"+owner_lbl.text+"/"+current_repo._name+"/contents/"+path,UserData.header,false,HTTPClient.METHOD_DELETE,JSON.print(body))
...
func request_file_content(path : String):
	...
	request.request("https://api.github.com/repos/"+owner_lbl.text+"/"+current_repo._name+"/contents/"+path+"?ref="+current_branch.name,UserData.header,false,HTTPClient.METHOD_GET)
	yield(self,"get_contents")

func _on_branch2_item_selected(ID):
	get_parent().loading(true)
	current_branch = branches_opt_btn.get_item_metadata(ID)
	request_contents(current_repo._name,current_branch)
	yield(self,"get_contents")
	build_list()
...
func on_newbranch_confirmed():
	...
	var body = {
			...
			"sha": from_branch.get_item_metadata(from_branch.get_selected_id()).commit.sha
	}

	request.request("https://api.github.com/repos/"+owner_lbl.text+"/"+current_repo._name+"/git/refs",UserData.header,false,HTTPClient.METHOD_POST,JSON.print(body))
...
func invite_collaborator():
	...
	if collaborator_name!="" and collaborator_name!=" ":
			...
			request.request("https://api.github.com/repos/"+owner_lbl.text+"/"+current_repo._name+"/collaborators/"+collaborator_name, header, false, HTTPClient.METHOD_PUT, JSON.print(body))
